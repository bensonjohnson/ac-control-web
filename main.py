...
pid = PID(1.0, 0.1, 0.05, setpoint=set_temperature)

def on_connect(client, userdata, flags, rc):
    # Subscribe to the temperature topics when connected to MQTT broker
...
def update_hvac_control():
    global fan_state, cooling_state, heating_state, set_temperature, external_temperature
    # Adjust the setpoint based on the external temperature
    adjusted_setpoint = set_temperature + external_temperature * 0.1
    pid.setpoint = adjusted_setpoint
    # Process the temperature difference and determine the HVAC control states
    control_value = pid(current_temperature)
...
def on_message(client, userdata, msg):
    ...
    # Update the current temperature, external temperature or set temperature when a new message is received
    if msg.topic == temperature_topic:
        ...
        try:
            ...
            print("External temperature updated:", external_temperature)
            update_hvac_control()  # Trigger the update_hvac_control() function
        except (ValueError, TypeError):
            print("Invalid external temperature payload received:", msg.payload)
